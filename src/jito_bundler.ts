...
const makeJitoPostRequest = async (payload: any, options = {}) => {
	for (let i = 0; i < JITO_BLOCK_ENGINES.length; i++) {
		try {
			const response = await axios.post(`https://${JITO_BLOCK_ENGINES[i]}/api/v1/bundles`, payload, options);
			return response;
		} catch (error) {
		}
	}
...
class JitoBundle {
	...
	public sendBundle = async (bundleTransactions: any[], feePayer: any, key: string): Promise<boolean> => {
		const wallet: any = getWalletFromPrivateKey(key)
		const seacher = searcher.searcherClient(
			this.engineURL,
			wallet.wallet
		);
		const tipAccounts = constants.JITO_TIP_ACCOUNTS;
		const tipAccount = new PublicKey(tipAccounts[utils.getRandomNumber(0, tipAccounts.length - 1)]);
...
export class JitoBundler {
	...
	public sendBundles = async (bundleTransactions: any[], payer: any, maxRetry: number = 2): Promise<boolean> => {
		...
		const result: boolean = await jitoBundle.sendBundle(bundleTransactions, payer ? payer.wallet : null, this.getJitoKey())
		maxRetry--
		if (!result && maxRetry) {
			await sleep(500)
			return await this.sendBundles(bundleTransactions.slice(0, len), payer, maxRetry)
		}
		return result
	}
}