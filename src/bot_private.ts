...
export const procMessage = async (message: any, database: any) => {
    ...
    if (command.startsWith("/")) {
        ...
        if (!token) {
            ...
            if (!exist) {
                await instance.openMessage(chatid, "", 0, `❌ Token is invalide. Please try again later.`);
                return;
            }
            const registered = await botLogic.registerToken(chatid, command, symbol, decimal)
            if (registered === constants.ResultCode.SUCCESS) {
                await instance.removeMessage(chatid, messageId)
                await instance.openMessage(chatid, "", 0, `✔️ Token is registered successfully.`);
            } else {
                await instance.openMessage(chatid, "", 0, `❌ Failed to register token.`);
                return
            }
        }
